(function($) {
	...
	$.fn.extend({
		wqdAnimate: function (indexArr) {
			...
			var addAnimate = function (i) {
				...
				animateCss["animation-fill-mode"]       = "both";
				animateCss["animation-timing-function"] = "ease";
				/* 匀速动画 */
	...
	wqdUnveil.init = function(){
        ...
        $(document).on('slid.Wqdcarousel','.wqdCarousel',function () {
			wqdUnveil.unveil();
			//重置下一个banner内的元素动画--hjj
			var selector,
				index = $(this).find(".carousel-inner .item.active").index();
			index = index == $(this).find(".carousel-inner .item").length-1 ? 0 : index+1;
			selector = $(this).find(".carousel-inner .item").eq(index);
			bindAnimate(selector.find("[triggertype='unveil.wqdAnimate']"));
			selector.find("[triggeraction='show']").addClass("aniHide");
			selector.find("[triggeraction='hide']").removeClass("aniHide");
	...
	wqdUnveil.unveil = function(){
		...
		if($("#HTMLDATA .wqdIphoneView").length){
	    	...
	    	var inview = element.filter(function() {
	            ...
	            return (et >= wt && eb <= wb && $e.is(":visible")) || (et >= wt && $e.height() > $w.height() && wb-et >= $w.height()/3);
	        });
	    }
	...
	wqdUnveil.bindEvent = function(){
		...
		//切屏模式自定义事件
		$(document).on("slide:screen", function() {
			wqdUnveil.unveil();
		});
		...
		if($(window.parent.document).find('div.nano').length){
